 ...
 * @LastEditTime: 2022-11-01 14:06:44
...
export default function NewsAdd(props) {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/news/${props.match.params.id}`)
      .then(res => {
  ...
  const nextStep = () => {
    // const {title,categories,content} = props
    if (currentStep === 0) {
  ...
  const submit = (auditState) => {
    ...
    axios.patch(`http://localhost:5000/news/${props.match.params.id}`, {
      ...
      notification.success({
        message: '通知',
        description:
          `新闻修改成功，你可以在${auditState === 0 ? '草稿箱' : '审核列表'}中查看你修改的新闻`,
      });
    })
  }
  ...
  return (
    <div>
      ...
      <div className={currentStep === 1 ? '' : style.isShow} style={{ marginTop: '20px' }}>
        <NewsEdit getContent={getContent} content={content}></NewsEdit>
      </div>
